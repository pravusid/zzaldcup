...
func (repo *MysqlCompetitorRepository) FindWithCursor(models interface{}, criteria *model.Competitor) error {
...
func (repo *MysqlCompetitorRepository) Count(count *int, criteria *model.Competitor) error {
	return repo.DefaultJob(func(db *gorm.DB) error {
		return db.Model(&model.Competitor{}).Where(criteria).Count(count).Error
	})
}

func (repo *MysqlCompetitorRepository) SaveAll(models []model.Competitor) error {
